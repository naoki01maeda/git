...
class BalancedPositiveNegativeSampler:
    ...
    def __call__(self, matched_idxs: List[Tensor]) -> Tuple[List[Tensor], List[Tensor]]:
        ...
        for matched_idxs_per_image in matched_idxs:
            positive = torch.where(matched_idxs_per_image > 0.)[0]#positive = torch.where(matched_idxs_per_image >= 1)[0]
            negative = torch.where(matched_idxs_per_image == 0.)[0]#negative = torch.where(matched_idxs_per_image == 0)[0]
