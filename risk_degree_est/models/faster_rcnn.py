...
#GeneralizedRCNNを継承している(流れが分かる)fowardがある
class FasterRCNN(GeneralizedRCNN):
    ...
    def __init__(
        ...
        box_batch_size_per_image=512,#画像1枚のbbox提案数
        box_positive_fraction=0.25,
        bbox_reg_weights=None,
        #デフォならNone, image_catなら"image_cat"にする
        add_features = "c3d_vit",
        **kwargs,
    ):
        ...
        rpn_pre_nms_top_n = dict(training=rpn_pre_nms_top_n_train, testing=rpn_pre_nms_top_n_test)#rpn_pre_nms_top_n_train(2000) rpn_pre_nms_top_n_test(512)
        rpn_post_nms_top_n = dict(training=rpn_post_nms_top_n_train, testing=rpn_post_nms_top_n_test)#2000, 512
        ...
        if box_predictor is None:
            ...
            if add_features == "image_cat":
                print("=============image_catモデルを学習します=============")
                representation_size = 2048
            if add_features == "vit":
                print("=============vitモデルを学習します=============")
                representation_size = 2048
            if add_features == "c3d_vit":
                print("=============c3d_vitモデルを学習します=============")
                representation_size = 2048
        ...
        super().__init__(backbone, rpn, roi_heads, transform)#これらの基底クラスのコンストラクタを継承(オーバーライド)
...
class FastRCNNPredictor(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.bbox_pred = nn.Linear(in_channels, 1 * 4)
